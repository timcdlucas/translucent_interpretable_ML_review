...
In figure \ref{fig:enethyp} we can see that the best performing model had mostly a LASSO penalty and an intermediate amount of regularization.
...
We can see (figure \ref{fig:gphyp}) that a relatively short scale length, which allows quite non-linear relationships, had a best model performance.
...
The number of randomly selected covariates to be considered at each split (mtry) was also selected by cross-validation.
We can see that high values of mtry and low values of minimum node size were selected as the best performing hyperparameters (figure \ref{fig:rfhyp}).
