...
In this review I review a number of techniques for interpreting machine learning models, discuss the limits of interpretability for different methods and demonstrate these approaches using a case example of understanding litter sizes in mammals.
...
Machine learning methods are a collection of techniques that focus on making accurate predictions from data \citep{crisci2012review, breiman2001statistical}.
...
They therefore estimate nonlinear relationships between covariates and response variables which are not summarised by a small number of interpretable parameters instead containing huge numbers of parameters often without estimates of uncertainty.
...
The nonparametric nature of many machine learning models means they can discover nonlinear relationships and interactions without specifying them \emph{a priori} as would be required in more statistical modelling.
Furthermore, they can be useful as a baseline to compare how well a mechanistic model performs.
...
\noindent\textsf{\textbf{Box 1: Types of machine learning model}}\scriptsize 
\linespread{0.1}
\vspace{-4mm}
...
More specifically, I examine how the models can be interpreted at the level of the full model, the individual covariate (with attention given to random effects) and finally at the level of the individual prediction.
...
Finally, the number of randomly selected covariates to be considered at each split (mtry) was also selected by cross-validation (figure \ref{fig:rfhyp}).
...
The Random Forest model fitted here has fairly good predictive performance (figure \ref{fig:rfpredobs}) with an \(r^2\) of 0.68.
...
Finally, the Random Forest model has two hyperparameters (figure \ref{fig:rfhyp}); mtry is the number of randomly selected covariates to be examined at each split in each tree and min.node.size is the maximum number of data points that can be in a leaf node of a tree.
...
\begin{figure}[t!]
  ...
  \label{fig:pdp}
  \caption{
    PDP plot for Gestation Length in a) the elastic net model, b) the Gaussian process model and c) the Random Forest model. The elastic net model can only fit linear relationships, the Gaussian process model fits smooth non-linear relationships while the Random Forest model fits more blocky relationships due to the decision trees that underpins it.
  }
\end{figure}
...
The mean response for each value of the covariate of interest is plotted.
...
Here we can clearly see the range of responses that exist for a single covariate, with Gestation length having a negative relationship with litter size in many cases but a flat relationship in others.
...
\begin{figure}[t!]
  ...
  \label{fig:ice}
  \caption{
    ICE plots for gestation length against litter size in a) the elastic net model, b) the Gaussian process model and c) the Random Forest model.
    Each grey curve is the relationship between gestation length and litter size evaluated at 10\% of the the datapoints. 
    The yellow curves are the median of these curves and is therefore the PDP as seen in Figure 3.
    Clustered ICE plot for gestation length against litter size in d) the Gaussian process model and e) the Random Forest model.
    The clustered ice plots are created by subtracting the mean of each curve and then clustering them using k-means.
    This normalisation step forces all the linear relationship in (a) to be identical and so the clustered plot is not shown. 
  }
\end{figure}
...
As a simple demonstration I fitted a phylogenetic linear model via INLA \citep{INLA} using the \emph{a priori} selected covariates  and a phylogenetic linear model using all covariates and strong regularising priors (\(r^2 = 0.74\)).
...
This fact is supported by the fact that the out-of-sample $r^2$ values are much higher in the phylogenetic models ($r^2 = 0.72$ and 0.74) than those of the non-phylogenetic models ($r^2 = 0.34$ and 0.53). 
...
\begin{figure}[t!]
  \centering
    \subfloat[Gaussian process model\protect\label{fig:2dgestlatenet}]{\includegraphics[width =0.33\textwidth ]{figs/enet_2d-1.pdf}}
