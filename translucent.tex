...
One notable difference between the LIME analysis for the Gaussian process model and the Random Forest model is that for the Gaussian process model, most covariates are in agreement that these datapoints should be high.
In contrast, there is a lot of contradiction between covariates in the Random Forest model.
For datapoint 1683 (figure \ref{fig:limerf}, left) the strongest single covariate contribution (body mass) is suggesting that these species should have a small litter size.
...
To conclude, we have seen a number of methods for interpreting machine learning models.
While these models are not as easy to interpret as carefully constrained statistical models, they make up for this by finding relationships that a simpler model would not be able to.
The interpretation of these models needs to done carefully and requiers user interaction and exploration in a way that is less vital in interpreting standard statistical models.
The power lies in the way a scientist defines their questions and uses these methods alongside other more statistical methods.
